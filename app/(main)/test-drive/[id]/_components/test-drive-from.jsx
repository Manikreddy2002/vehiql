...
const TestDriveForm = ({ car, testDriveInfo }) => {
  ...
  const generateTimeSlots = useCallback((date) => {
    ...
    // If no dealership data, provide default working hours (9 AM to 6 PM)
    let daySchedule = null;
    if (dealership?.workingHours) {
      const selectedDayOfWeek = format(date, "EEEE").toUpperCase();
      daySchedule = dealership.workingHours.find(
  ...
  const isDayDisabled = (day) => {
    ...
    // If no dealership data, allow all future dates
    if (!dealership?.workingHours) {
      return false;
    }
          ...
          {bookingDetails && (
            <div className="py-4 flex flex-col gap-3">
              <div className="space-y-3">
                <span className="flex justify-between items-center w-full p-3">
                  <span className="font-medium">Car:</span>
                  <span className="font-semibold whitespace-nowrap w-full text-right ml-4">{car.year} {car.make} {car.model}</span>
                </span>
                <span className="flex justify-between items-center w-full p-3">
                  <span className="font-medium">Date:</span>
                  <span className="font-semibold whitespace-nowrap w-full text-right ml-4">{bookingDetails.date}</span>
                </span>
                <span className="flex justify-between items-center w-full p-3">
                  <span className="font-medium">Time Slot:</span>
                  <span className="font-semibold whitespace-nowrap w-full text-right ml-4">{bookingDetails.timeSlot}</span>
                </span>
                <span className="flex justify-between items-center w-full p-3">
                  <span className="font-medium">Dealership:</span>
                  <span className="font-semibold whitespace-nowrap w-full text-right ml-4">{dealership?.name || "Vehiql Motors"}</span>
                </span>
              </div>
              <span className="mt-4 bg-blue-50 text-sm text-blue-700 dialog-3d-animate p-3">
